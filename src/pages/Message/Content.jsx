...
function ContentMessage() {
  ...
  const save = async (data) => {
    ...
    if (!toKnownChecked.value)
      return toast.error(
        "Aucun contact ou liste de diffusion n'est associé à ce message !"
      );
    ...
    return axios.post(ApiUrl + "message/create", formData, {
  ...
  const { mutate: messageRegister } = useMutation({
    ...
    onError: (error) => {
      document
        .querySelector(".FormSaveContact")
        .querySelectorAll("input", "buttton")
  ...
  const handleChange = (event) => {
    ...
    const ParentChecked =
      document.querySelector(".ListingContacts") ||
      document.querySelector(".ListingGroupes");
    if (name == "contact" && event.target.checked)
      valueInput.toKnownChecked = "forContact";
    else if (name == "groupe" && event.target.checked)
      valueInput.toKnownChecked = "forGroupe";
    if (ParentChecked) {
      const inputChecked = ParentChecked.querySelectorAll(
  ...
  return (
    <div className="container flex mx-auto m-4">
      <div className="w-full ">
        <form
          className="bg-white shadow-md rounded px-8 pt-6"
          onSubmit={handleSubmit(onSubmit)}
          encType="multipart/form-data"
        >
          <h2 className="mb-5 text-gray-900 font-bold text-center">
            Contenu Message
          </h2>
          <div className="mb-5">
            <div>
          ...
          <div className="m-2">
            ...
            />
            {errors.object && (
              <p className="text-red-500 text-sm">{errors.object.message}</p>
            )}
          </div>
          <div className="m-2">
            <label
            ...
            ></textarea>
            {errors.contenu && (
              <p className="text-red-500 text-sm">{errors.contenu.message}</p>
            )}
          </div>
          ...
          <div className="m-2 md:flex md:items-center items-center justify-center grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              ...
              <div className="flex flex-wrap">
                <label
                  className="mb-2 text-[#1d1b31] hover:text-blue-700 font-bold mr-9"
                  htmlFor="sms"
                >
                  <input
                    type="radio"
              ...
              {errors.canal && (
                <p className="text-red-500 text-sm">{errors.canal.message}</p>
              )}
            </div>
          ...
          <div className="flex m-2 justify-center">
            <button
              className="bg-[#1d1b31] hover:bg-blue-700 text-white font-bold py-2 px-4 rounded focus:outline-none focus:shadow-outline mb-4"
              type="button"
